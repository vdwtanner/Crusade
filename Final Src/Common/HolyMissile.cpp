...
HolyMissile::HolyMissile(UINT id, std::string tag, std::string name, Ability::AbilityDescription* desc)
	: Ability(id, tag, name, desc)
{
	//Holy missile is a single stage spell as of now, but if we add special things we can add them here
...
HolyMissile* HolyMissile::clone(){
	HolyMissile* clone = new HolyMissile(m_id, m_tag, m_name, &getAbilityDesc());
	return clone;
}

// gets called during the next update loop after the manager creates an ability
void HolyMissile::initSensorBody(){
...
void HolyMissile::step(float dt){
...
void HolyMissile::onTrigger(GameObject* collider){
	if (collider == m_owner){
		return;
	}
