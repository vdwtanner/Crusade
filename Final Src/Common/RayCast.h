...
public:
	...
	float32 ReportFixture(b2Fixture* fixture, const b2Vec2& point, const b2Vec2& normal, float32 fraction)
	{
		...
		void* userData = m_body->GetUserData();
		if (userData)
		{
			GameObject* g = (GameObject*)userData;
			if (g->getTag().compare("WORLDTILE") == 0)
			{
				WorldTile* t = (WorldTile*)userData;
				if (t->getId() != 0){
					m_hit = true;
					m_point = point;
					m_normal = normal;
			...
			else if(g->getTag().compare("ENEMY") == 0){
				m_hit = true;
				m_point = point;
				m_normal = normal;
				return fraction;
	...
	GameObject* getGameObject(){
		if (m_body->GetUserData() == nullptr){
			return nullptr;
		}
		return (GameObject*)m_body->GetUserData();
	}
...
class RayCastClosestCallbackIgnoreMe : public b2RayCastCallback
{
public:
	RayCastClosestCallbackIgnoreMe(GameObject* me)
	{
		m_hit = false;
		m_body = nullptr;
		m_me = me;
...
/*A RayCast callback that specifically checks for the player within the ray*/
class RayCastLookForPlayer : public b2RayCastCallback
{
public:
	RayCastLookForPlayer()
	{
		m_hit = false;
		m_body = nullptr;
		m_fraction = -1;
	...
	float32 ReportFixture(b2Fixture* fixture, const b2Vec2& point, const b2Vec2& normal, float32 fraction)
	{
		...
		if (userData)
		{
			...
			else if (g->getTag().compare("PLAYER") == 0){
				m_hit = true;
				m_point = point;
				m_normal = normal;
				m_fraction = fraction;
	...
	Character* getPlayer(){
		if (m_body->GetUserData() == nullptr || !m_hit){
			return nullptr;
		}
		return (Character*)m_body->GetUserData();
	}
