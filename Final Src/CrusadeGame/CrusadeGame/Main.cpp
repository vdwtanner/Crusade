//
// Main.cpp
// Based on DirectxTK tutorials at https://directxtk.codeplex.com/documentation
//
...
int WINAPI wWinMain(_In_ HINSTANCE hInstance, _In_opt_ HINSTANCE hPrevInstance, _In_ LPWSTR lpCmdLine, _In_ int nCmdShow)
{
	...
	HDEVNOTIFY hNewAudio = nullptr;
	{
        ...
        wcex.hIcon = LoadIcon(hInstance, L"IDI_ICON2");
        wcex.hCursor = LoadCursor(nullptr, IDC_ARROW);
        wcex.hbrBackground = (HBRUSH) (COLOR_WINDOW + 1);
        wcex.lpszMenuName = nullptr;
        wcex.lpszClassName = L"CrusadeGameWindowClass";
        ...
        HWND hwnd = CreateWindow(L"CrusadeGameWindowClass", L"CrusadeGame", WS_OVERLAPPEDWINDOW,
    ...
    while (WM_QUIT != msg.message)
    {
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE))
        {
			...
			if (WM_CHAR && g_game->getCurrentScreen() == Game::PROFILE){
				g_game->getProfileScreen()->handleWM_CHAR(msg);
				g_game->Tick();
				if (g_game->exitCrusade()){
					break;
				}
			}
			if (WM_KEYDOWN){
				g_game->handleKeyPress(msg);
				g_game->Tick();
				if (g_game->exitCrusade()){
					break;
				}
